...
checkService()
{
    ...
    if [ -n "$serviceAlias" ]; then
        echo -e "\n[+] $serviceToCheckExists is on this machine\n"
        if echo "$serviceList" | grep -qi "$serviceAlias\|$serviceToCheckExists" ; then
            if [ "$( DPRINT netstat -tulpn | grep -i $serviceAlias )" ] ; then
                
                echo -e "Active on port(s) $(netstat -tulpn | grep -i "$serviceAlias\|$serviceToCheckExists"| awk 'BEGIN {ORS=" and "} {print $1, $4}' | sed 's/\(.*\)and /\1\n/')\n"
            
            elif [ "$( DPRINT ss -blunt -p | grep -i $serviceAlias )" ] ; then
                
                echo -e "Active on port(s) $(ss -blunt -p | grep -i "$serviceAlias\|$serviceToCheckExists"| awk 'BEGIN {ORS=" and " } {print $1,$5}' | sed 's/\(.*\)and /\1\n/')\n"
            fi
        ...
        if [ "$( DPRINT netstat -tulpn | grep -i $serviceToCheckExists )" ] ; then
                ...
                echo -e "Active on port(s) $(netstat -tulpn | grep -i $serviceToCheckExists| awk 'BEGIN {ORS=" and "} {print $1, $4}' | sed 's/\(.*\)and /\1\n/')\n"
        
        elif [ "$( DPRINT ss -blunt -p | grep -i $serviceToCheckExists )" ] ; then
                
                echo -e "Active on port(s) $(ss -blunt -p | grep -i $serviceToCheckExists| awk 'BEGIN {ORS=" and " } {print $1,$5}' | sed 's/\(.*\)and /\1\n/')\n"
        fi
    fi
}
    ...
    if [ -n "$VOLUMES" ]; then
        ...
        for v in $VOLUMES; do
            ...
            if [ -n "$container" ]; then
                ...
                echo -e "$v -> $mountpoint used by $container"
            fi
        done
        echo ""
    fi
...
if checkService "$SERVICES"  'cockpit' | grep -qi "is on this machine"; then
    ...
    echo -e "[!] WE PROBABLY SHOULD KILL COCKPIT"
fi
...
sql_test(){
    ...
    if [ -n "$DEFAULT_PASS" ]; then
        if mysql -uroot -p"$DEFAULT_PASS" -e 'bruh' 2>&1 >/dev/null | grep -v '\[Warning\]' | grep -q 'bruh'; then
            echo -e "Can login with root:$DEFAULT_PASS\n"
    ...
    if [ "$SQL_AUTH" = 1 ]; then
        ...
        echo -e "$( DPRINT $SQLCMD -t -e 'select user,host,plugin,authentication_string from mysql.user where password_expired="N";' )\n" 
